...
#define __HAL_PWR_VOLTAGESCALING_CONFIG(__REGULATOR__) do {                                                     \
                                                            ...
                                                            MODIFY_REG(PWR->CR.reg, PWR_CR_VOS, (__REGULATOR__));   \
                                                            /* Delay after an RCC peripheral clock enabling */  \
                                                            tmpreg = READ_BIT(PWR->CR.reg, PWR_CR_VOS);             \
                                                            UNUSED(tmpreg);                                     \
                                                          } while(0U)
#else
/** @brief  macros configure the main internal regulator output voltage.
...
/** @brief Macros to enable or disable the Under drive mode.
  ...
  */
#define __HAL_PWR_UNDERDRIVE_ENABLE() (PWR->CR.reg |= (uint32_t)PWR_CR_UDEN)
#define __HAL_PWR_UNDERDRIVE_DISABLE() (PWR->CR.reg &= (uint32_t)(~PWR_CR_UDEN))
