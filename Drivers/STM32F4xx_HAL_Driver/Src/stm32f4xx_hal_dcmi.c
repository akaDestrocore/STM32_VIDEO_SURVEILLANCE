...
HAL_StatusTypeDef HAL_DCMI_Suspend(DCMI_HandleTypeDef* hdcmi)
{
  ...
  if(hdcmi->State == HAL_DCMI_STATE_BUSY)
  {
    ...
    while((hdcmi->Instance->CR.reg & DCMI_CR_CAPTURE) != 0);
  }    
  /* Process Unlocked */
  __HAL_UNLOCK(hdcmi);
...
HAL_StatusTypeDef HAL_DCMI_ConfigCrop(DCMI_HandleTypeDef *hdcmi, uint32_t X0, uint32_t Y0, uint32_t XSize, uint32_t YSize)
{
  ...
  /* Configure CROP */
  hdcmi->Instance->CWSIZE.reg = (XSize | (YSize << DCMI_POSITION_CWSIZE_VLINE));
  hdcmi->Instance->CWSTRT.reg = (X0 | (Y0 << DCMI_POSITION_CWSTRT_VST));
...
HAL_StatusTypeDef HAL_DCMI_DisableCrop(DCMI_HandleTypeDef *hdcmi)
{
  ...
  hdcmi->Instance->CR.reg &= ~(uint32_t)DCMI_CR_CROP;
...
HAL_StatusTypeDef  HAL_DCMI_ConfigSyncUnmask(DCMI_HandleTypeDef *hdcmi, DCMI_SyncUnmaskTypeDef *SyncUnmask)
{
  ...
  hdcmi->Instance->ESUR.reg = (((uint32_t)SyncUnmask->FrameStartUnmask) |\
