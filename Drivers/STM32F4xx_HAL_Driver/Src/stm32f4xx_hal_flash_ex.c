    ...
    *SectorError = 0xFFFFFFFFU;
    ...
    else
    {
      ...
      for (index = pEraseInit->Sector; index < (pEraseInit->NbSectors + pEraseInit->Sector); index++)
      {
        ...
        CLEAR_BIT(FLASH->CR.reg, (FLASH_CR_SER | FLASH_CR_SNB));
...
static void FLASH_MassErase(uint8_t VoltageRange, uint32_t Banks)
{
  ...
  CLEAR_BIT(FLASH->CR.reg, FLASH_CR_PSIZE);
  ...
  else if (Banks == FLASH_BANK_1)
  {
    ...
    FLASH->CR.reg |= FLASH_CR_MER1;
  }
  else
  {
    /*Only bank2 will be erased*/
    FLASH->CR.reg |= FLASH_CR_MER2;
  }
  FLASH->CR.reg |= FLASH_CR_STRT | ((uint32_t)VoltageRange << 8U);
}
...
void FLASH_Erase_Sector(uint32_t Sector, uint8_t VoltageRange)
{
  ...
  CLEAR_BIT(FLASH->CR.reg, FLASH_CR_PSIZE);
  FLASH->CR.reg |= tmp_psize;
  CLEAR_BIT(FLASH->CR.reg, FLASH_CR_SNB);
  FLASH->CR.reg |= FLASH_CR_SER | (Sector << FLASH_CR_SNB_Pos);
  FLASH->CR.reg |= FLASH_CR_STRT;
}
...
static uint8_t FLASH_OB_GetUser(void)
{
  ...
  return ((uint8_t)(FLASH->OPTCR.reg & 0xE0));
}
...
void FLASH_FlushCaches(void)
{
  ...
  if (READ_BIT(FLASH->ACR.reg, FLASH_ACR_ICEN) != RESET)
  {
  ...
  /* Flush data cache */
  if (READ_BIT(FLASH->ACR.reg, FLASH_ACR_DCEN) != RESET)
  {
    /* Disable data cache  */
