...
void HAL_PWR_ConfigPVD(PWR_PVDTypeDef *sConfigPVD)
{
  ...
  MODIFY_REG(PWR->CR.reg, PWR_CR_PLS, sConfigPVD->PVDLevel);
...
void HAL_PWR_EnableWakeUpPin(uint32_t WakeUpPinx)
{
  ...
  SET_BIT(PWR->CSR.reg, WakeUpPinx);
}
...
void HAL_PWR_DisableWakeUpPin(uint32_t WakeUpPinx)
{
  ...
  CLEAR_BIT(PWR->CSR.reg, WakeUpPinx);
}
...
void HAL_PWR_EnterSTOPMode(uint32_t Regulator, uint8_t STOPEntry)
{
  ...
  MODIFY_REG(PWR->CR.reg, (PWR_CR_PDDS | PWR_CR_LPDS), Regulator);
...
{
  /* Select Standby mode */
  SET_BIT(PWR->CR.reg, PWR_CR_PDDS);
