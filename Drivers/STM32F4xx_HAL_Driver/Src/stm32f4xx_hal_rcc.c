...
uint32_t HAL_RCC_GetPCLK2Freq(void)
{
  ...
  return (HAL_RCC_GetHCLKFreq()>> APBPrescTable[(RCC->CFGR.reg & RCC_CFGR_PPRE2)>> RCC_CFGR_PPRE2_Pos]);
}
...
void HAL_RCC_GetClockConfig(RCC_ClkInitTypeDef  *RCC_ClkInitStruct, uint32_t *pFLatency)
{
  ...
  RCC_ClkInitStruct->SYSCLKSource = (uint32_t)(RCC->CFGR.reg & RCC_CFGR_SW);

  /* Get the HCLK configuration ----------------------------------------------*/
  RCC_ClkInitStruct->AHBCLKDivider = (uint32_t)(RCC->CFGR.reg & RCC_CFGR_HPRE);

  /* Get the APB1 configuration ----------------------------------------------*/
  RCC_ClkInitStruct->APB1CLKDivider = (uint32_t)(RCC->CFGR.reg & RCC_CFGR_PPRE1);

  /* Get the APB2 configuration ----------------------------------------------*/
  RCC_ClkInitStruct->APB2CLKDivider = (uint32_t)((RCC->CFGR.reg & RCC_CFGR_PPRE2) >> 3U);

  /* Get the Flash Wait State (Latency) configuration ------------------------*/
  *pFLatency = (uint32_t)(FLASH->ACR.reg & FLASH_ACR_LATENCY);
}
